...
class CloudProvider(models.Model):
    provider = models.CharField(max_length=4, unique=True, validators=[validators.MinLengthValidator(3)], primary_key=True)
...
class CloudRegion(models.Model):
    ...
    geographic_region = models.CharField(max_length=32, validators=[validators.MinLengthValidator(2)]) # europe
    cardinality = models.CharField(max_length=12, validators=[validators.MinLengthValidator(2)]) # west
    ...
    def natural_key(self):
        return (self.provider.pk, self.original_region_name,)
...
class CloudAvailabilityZoneManager(models.Manager):
    def get_by_natural_key(self, az, region_natural_key):
        return self.get(az=az, 
                        region=CloudRegion.objects.get_by_natural_key(*region_natural_key))
...
class CloudAvailabilityZone(models.Model):
    ...
    def natural_key(self):
        return (self.az, self.region.natural_key(),)
